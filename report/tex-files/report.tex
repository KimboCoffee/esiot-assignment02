...
\begin{document}
	...
	\newline In particular, this system runs on a round-robin cooperative scheduler. This means that - each period - control is given in turn to all tasks, which have to atomically perform a finite step, since the scheduler doesn't stop its execution as it does in preemptive scheduling. This is easier to implement, and can be achieved in this system because every task has relatively simple operations to perform at each step, and therefore is considered to be non-blocking. The main consequence of implementing a round-robin scheduler is that it defines implicit priorities between Tasks, since control is given in a certain order at each period. This has to be taken into account when dealing with inter-task communication, since the sources of messages have to synchronize with their receivers. Another important concern is the choice of the period: each period must be long enough to allow all Tasks to execute, but it also has to be an exact divisor of all Tasks' own periods. This because otherwise it would mean losing responsiveness to Task operations. In particular, an ideal period for responsiveness would be the Greatest Common Divisor between all Task periods, but on the other hand it might result in a time too short to allow each Task to execute its step (note that in the worst case scenario, which certainly happens at some time in execution, all Tasks have to perform their step in a single period). In this system though, since there are only three Tasks that can have relatively long periods (tenths to hundreds of milliseconds), it is not really a big issue.
	...
	\newline To realize the Servo Motor controlled door, the \href{https://docs.arduino.cc/libraries/servo/}{Servo library} was used. This library provides a wrapper class whose instances are the motors connected to the board. In this way, they can be controlled abstracting from the low level aspects of digital communication behind their functioning. In particular, this library exploits Timer 1 to achieve motor controlling, and this can become problematic since it disables PWM controlled by that timer, and generally speaking interferes with other potential features that would exploit it. This has obviously to be taken into account, but doesn't really become an issue in this project because the scheduler period is short enough to be effectively handled by any of the other two timers, which are 8bit instead of 16bit. In this way, time is controlled effectively, and yet timer 1 is left free to control the servo motor thanks to this library.
