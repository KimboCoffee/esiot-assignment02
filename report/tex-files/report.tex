...
\begin{document}
	...
	\newline For what concerns Tasks, together with the three main Tasks depicted in the Analysis, two other have to be implemented. The first is a communication Task, which is responsible of all the actions needed - board side - to communicate on the serial line. This Task will focus on sending data to the Dashboard, and collecting its input to modify the system state. Another important Task, directly linked to this one, is the one that tracks the system's state. In particular, this is used for inter-Task communication, so that when an exception occurs the whole system can be set to non-working in order to block the main Task's procedures. On the other hand, it's reactive to Dashboard input in order to restore the correct functionality of the main system.
