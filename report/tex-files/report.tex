...
\begin{document}
	...
	From an implementing standpoint, the System consists on three main modules: a Message Handler, the App and the GUI. The Message Handler, based on serial communication, reads the period at startup, and then each period it polls for a fixed-structure message containing the temperature, the filling level, and two flags eventually signaling the respective sensor exceptions. This is then processed by the App, that handles the information to update the GUI accordingly.
